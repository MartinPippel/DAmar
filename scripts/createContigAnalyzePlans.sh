...
function setForcealignOptions()
{
    ...
    if [[ -n ${COR_CONTIG_FORCEALIGN_CORRELATION} && ${COR_CONTIG_FORCEALIGN_CORRELATION} -gt 0 ]]
    then 
...
function setCTanalyzeOptions()
{
    ...
    if [[ -n ${COR_CONTIG_CTANALYZE_EXPROWREADCOV} && ${COR_CONTIG_CTANALYZE_EXPROWREADCOV} -gt 0 ]]
    then
        CONTIG_CTANALYZE_OPT="${CONTIG_CTANALYZE_OPT} -c ${COR_CONTIG_CTANALYZE_EXPROWREADCOV}"
    fi
...
then 
    ...
    then
        ...
        # create dazzler db
    	first=1 
        for x in ${FIX_FILT_OUTDIR}/${ANALYZE_DIR}/correctedContigs_dazzler/*.fasta
        do
        	if [[ ! -f ${x} ]]
			then 
	        ...
	        then
	        	first=0
				echo "${DAZZLER_PATH}/bin/fasta2DB -v ${FIX_FILT_OUTDIR}/${ANALYZE_DIR}/${CONT_DAZZ_DB} ${x}"
    ...
    then 
        ...
        for x in $(seq 1 ${contigblocks})
        do 
            echo "cd $(pwd)/${FIX_FILT_OUTDIR}/${ANALYZE_DIR} && ${MARVEL_PATH}/bin/datander${CONTIG_DATANDER_OPT} ${FIX_FILT_OUTDIR}/${ANALYZE_DIR}/${CONT_DB%.db}.${x}"
    ...
    then
        ...
        do 
            ...
            cmd="cd $(pwd)/${FIX_FILT_OUTDIR}/${ANALYZE_DIR} && ${NUMACTL}${MARVEL_PATH}/bin/daligner${CONTIG_DALIGNER_OPT} ${FIX_FILT_OUTDIR}/${ANALYZE_DIR}/${CONT_DB%.db}.${x}"
            cmdLine=$((${cmdLine}+1))
            count=0
            ...
            do  
                ...
                then
                    ...
                    cmd="cd $(pwd)/${FIX_FILT_OUTDIR}/${ANALYZE_DIR} && ${NUMACTL}${MARVEL_PATH}/bin/daligner${CONTIG_DALIGNER_OPT} ${FIX_FILT_OUTDIR}/${ANALYZE_DIR}/${CONT_DB%.db}.${x} ${FIX_FILT_OUTDIR}/${ANALYZE_DIR}/${CONT_DB%.db}.${y}"
                    cmdLine=$((${cmdLine}+1))
                    count=1
                fi
            done
